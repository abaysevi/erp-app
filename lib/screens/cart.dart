...
class _CartPageState extends State<CartPage> {
  ...
  Future<void> _processPayment() async {
    ...
    if (_cartItems.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Cart is empty. Add items!'),
        ),
      );
      return; // Stop further execution if the cart is empty
    }

    // Add purchased items to the sales table with the same order ID
    await SQLHelper.createSale(
      _cartItems,
      _calculateTotalAmount(),
      _selectedPaymentOption?.name ?? 'Cash',
    );
    Navigator.push<void>(
      context,
        ...
        child: Padding(
          ...
          child: Column(
            children: [
              ...
              const SizedBox(height: 16), // Adjust the spacing between the rows
              Row(
                mainAxisAlignment: MainAxisAlignment.end,
                children: [
                  DropdownButton<PaymentOption>(
                    value:
                        _selectedPaymentOption, // No need for null check, already initialized
